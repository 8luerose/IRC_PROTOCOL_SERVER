...
Server::~Server()
{
    ...
    for (std::map<std::string, Channel *>::iterator iter2 = _channelList.begin(); iter2 != _channelList.end(); iter2++)
    {
        delete iter2->second;
    }
    _clients.clear();
...
void Server::run()
{
    ...
    addEvent(_serverSock, EVFILT_READ, EV_ADD | EV_ENABLE);
...
void Server::execute()
{
    while (true)
    {
        int nev = kevent(_kq, _changeList, _changeListSize, _eventList, 256, NULL);
        _changeListSize = 0;
        ...
        for (int i = 0; i < nev; i++)
        {
            struct kevent *curr_event = &_eventList[i];

            if (curr_event->flags & EV_ERROR)
            {
                std::cerr << "Event error: " << strerror(curr_event->data) << std::endl;
                continue;
            }

            if (curr_event->filter == EVFILT_READ)
            {
                if (curr_event->ident == (uintptr_t)_serverSock)
                {
                    int clientSock = accept(_serverSock, NULL, NULL);
                    if (clientSock == -1)
                        std::cerr << "accept() failed" << std::endl;
                    else
                    {
                        fcntl(clientSock, F_SETFL, O_NONBLOCK);
                        addEvent(clientSock, EVFILT_READ, EV_ADD | EV_ENABLE);
                ...
                {
                    int clientSock = curr_event->ident;
                    int strLen = recvMessage(clientSock);
                    if (strLen <= 0)
                    {
                        std::cout << "Client disconnected: " << clientSock << std::endl;
                        _clients.erase(clientSock);
                        close(clientSock);
                        removeEvent(clientSock, EVFILT_READ);
                    }
                    else
                    {
                        if (checkMessageEnds(clientSock))
                        {
                            doCommand(clientSock);
                            _message[clientSock] = "";
                        }
                    }
                }
...
void Server::addEvent(int fd, int16_t filter, uint16_t flags)
{
    EV_SET(&_changeList[_changeListSize++], fd, filter, flags, 0, 0, NULL);
}

void Server::removeEvent(int fd, int16_t filter)
{
    EV_SET(&_changeList[_changeListSize++], fd, filter, EV_DELETE, 0, 0, NULL);
}
