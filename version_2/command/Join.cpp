...
void Command::join(int fd, std::vector<std::string> cmdVector)
{
	...
	while (iter != joinChannelArgv.end())
	{
		...
		if (channelIt != channelList.end())
		{
			...
			// 태현 추가
			// 1. std::map<std::string, Channel*> &channelList = _server.getChannelList();
			// 2. 서버가 갖고 있는 채널리스트에서 -> channelList.find(*iter) == channelList.find(#general)) -> 채널 찾은 반환값이 'channelIt'
			// 3. #general,#random .. 순서대로 훑어보며 위치 반환한 것이 'iter'
		...
		else // 채널이 존재하지 않을 경우
		{
			...
			// channelList.insert(std::make_pair(*iter, &channel));
			// channel.appendFdListClient(fd);
			// messageAllChannel(fd, "JOIN", *iter, ", ");
